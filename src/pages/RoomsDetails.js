...
const RoomsDetails = () => {
  ...
  const { id } = useParams();
  const incrementedId = Number(id) + 1;
  const room = rooms.find((room) => {
    return room.id == Number(id);
  });

  const room2 = rooms.find((room) => {
    return room.id == Number(incrementedId);
  });
  const { name, description, facilities, imageLg, price } = room;
  const { name2, description2, facilities2, imageLg2, price2 } = room2;
  console.log(room2);
  return (
    <section className=''>
      {/* {banner} */}
      ...
      <div className='container mx-auto'>
        <div className='flex felx-col lg:flex-row h-full py-24'>
          ...
          {/* left */}
          <div className='w-full h-full lg:w-[60%] px-6 '>
            <h2 className='h2 '>Signature Room</h2>
            <p>{description}</p>
            <img src={imageLg} alt='' />
            {/* facilities */}
            <div className='mt-12'>
